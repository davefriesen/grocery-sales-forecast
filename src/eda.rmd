...
# Option 1: Foward and backward fill for missing values
# oil_df2: New 'oil_df' data frame without missing values
oil_df2 <- na.locf(oil_df, fromLast = TRUE)

# Option 2: Drop missing records since they only account for 2% of data
# na.omit(oil_df)
...
sales_wk_df <- as.data.frame(sales_core_df %>%
                               mutate(year = year(date), week = week(date)) %>%
                               group_by(year, week) %>%
                               summarize(sales = sum(sales / 1000.0), dcoilwtico = mean(dcoilwtico)))
...
###### Model-Driven: Regression w/Autocorrelation
...
###### Model-Driven: Regression w/Autocorrelation - AR(1)
...
###### Model-Driven: Regression w/Autocorrelation - ARIMA
...
###### Model-Driven: Regression w/External Information
...
###### Model-Driven: Regression w/Autocorrelation - ARIMA + External Predictor(r)

```{r}
# Identify outcome (target) - sales
sales_train_y <- ts(sales_wk_df$sales, freq = 52)

# Identify external predictor(s) - oil price
sales_X <- c("dcoilwtico")
sales_train_X <- as.matrix(sales_wk_df[ , sales_X])
...
# Fit ARIMA model
#**********NOTE HAVE BUG IN ARIMA W/EXTERNAL PREDICTOR**********
sales_am <- Arima(sales_train_y, order = c(0, 0, 1))
#sales_amX <- Arima(sales_train_y, order = c(0, 0, 1), xreg = sales_train_X)
summary(sales_am)
#summary(sales_amX)
```


###### Model-Driven: Neural Network Autoregression
...
text(x = "MAE", y = "", labels = results_df$MAE)
```

```{r}
ggplot(results_df, aes(x = MAE, y = MAE)) +
  geom_point() +
...
results_df_reshape = reshape2::melt(results_df, c("Model"))
